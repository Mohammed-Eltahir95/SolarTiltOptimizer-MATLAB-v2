...
raw=webread(endpoint,params{:},weboptions('Timeout',120,'ContentType','json'));if product=="hourly",rows=raw.outputs.hourly;else,rows=raw.outputs.tmy_hourly;end
weather=solartilt.data.normalizePVGIS(rows,location);provenance=struct('cacheKey',key,'sourceType','PVGIS','sourceUri',endpoint,'apiVersion','5.3','dataset',string(source.radiationDatabase),'product',product,'startYear',source.startYear,'endYear',source.endYear,'retrievedAt',datetime('now','TimeZone','UTC'),'locationName',string(location.name));
end
